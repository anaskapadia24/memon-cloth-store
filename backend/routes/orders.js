...
// Adjust product stock for each order item (color + size aware).
// restore=false reduces stock (order placed), restore=true adds it back (order cancelled).
async function adjustStock(items, restore) {
  for (const item of items) {
    ...
    if (item.color && product.colors && product.colors.length > 0) {
      ...
      if (colorObj) {
        ...
        if (sizeObj) {
          sizeObj.stock = Math.max(0, sizeObj.stock + delta);
        } else if (restore) {
          colorObj.stock = (colorObj.stock || 0) + item.qty;
        }
      }
    } else if (item.size) {
      const sizeObj = product.sizes.find((s) => s.size === item.size);
      if (sizeObj) sizeObj.stock = Math.max(0, sizeObj.stock + delta);
    } else {
...
router.post("/", auth, async (req, res) => {
  try {
    const { customer, items, payment, notes, paymentId, coupon } = req.body;
    ...
    if (
      ...
      return res
        .status(400)
        .json({ error: "Complete delivery details are required" });
    }
    ...
    if (
      items.some((item) => !item.name || !(item.qty > 0) || !(item.price >= 0))
    ) {
      return res
        .status(400)
        .json({ error: "Each item needs a name, quantity, and price" });
    }
    if (!payment) {
    ...
    const quote = await quoteCheckout({
      ...
      city: customer.city,
      pin: customer.pin,
      payment: paymentId ? "online" : payment,
      userId: req.user._id,
    });
    if (coupon && quote.error) {
    ...
    const order = new Order({
      ...
      promoTitle: quote.promo ? quote.promo.title : "",
      coupon: quote.promo && quote.promo.coupon ? quote.promo.coupon : "",
      ...
      payment,
      paymentId: paymentId || "",
      paymentStatus: paymentId ? "paid" : payment === "cod" ? "cod" : "unpaid",
      notes,
...
// Public / customer track by AWB or last 8 of order id
router.get("/track", async (req, res) => {
  try {
    const awb = (req.query.awb || "").trim();
    const code = (req.query.order || "").trim().toUpperCase();
    let order = null;
    if (awb) order = await Order.findOne({ "shiprocket.awbCode": awb });
    if (!order && code) {
      const all = await Order.find({}).select(
        "_id status tracking shiprocket customer.name createdAt items total",
      );
      order = all.find((o) => orderId(o) === code) || null;
    }
    if (!order)
      return res
        .status(404)
        .json({ error: "No shipment found for that number" });

    if (order.shiprocket && order.shiprocket.awbCode) {
      try {
        const live = await shiprocket.trackByAwb(order.shiprocket.awbCode);
        return res.json({
...
router.post("/:id/refresh-track", adminAuth, async (req, res) => {
  try {
    ...
    if (!order.shiprocket || !order.shiprocket.awbCode) {
      return res
        .status(400)
        .json({ error: "This order has no Shiprocket AWB yet" });
    }
    const live = await shiprocket.trackByAwb(order.shiprocket.awbCode);
    const current =
      live?.tracking_data?.shipment_status ||
      live?.tracking_data?.track_status ||
      "";
    if (current) order.shiprocket.status = String(current);
    await order.save();
    res.json({ order, live });
  } catch (err) {
...
router.post("/:id/return-request", auth, async (req, res) => {
  try {
    ...
    if (!["replace", "refund"].includes(type)) {
      return res
        .status(400)
        .json({ error: 'Type must be "replace" or "refund"' });
    }
    ...
    if (order.status !== "delivered") {
      return res
        .status(400)
        .json({ error: "Returns are only available for delivered orders" });
    }
    ...
    if (daysSinceOrder > 7) {
      return res
        .status(400)
        .json({ error: "Return window (7 days) has expired for this order" });
    }
...
router.put("/:id/return-status", adminAuth, async (req, res) => {
  try {
    ...
    if (!order.returnRequest || !order.returnRequest.requested) {
      return res
        .status(400)
        .json({ error: "No return request exists for this order" });
    }
      ...
      try {
        ...
        if (returnData.shipment_id) {
          ...
          if (awbData.response && awbData.response.data) {
            order.returnRequest.returnAwbCode =
              awbData.response.data.awb_code || "";
            order.returnRequest.returnCourierName =
              awbData.response.data.courier_name || "";
          }
        }
      } catch (pickupErr) {
        console.error(
          "Reverse pickup booking error (non-fatal):",
          pickupErr.response?.data || pickupErr.message,
        );
      }
    }
...
router.get("/:id/invoice", adminAuth, async (req, res) => {
  try {
    ...
    doc
      ...
      .font("Helvetica")
      .fillColor("#666")
      .text(`Date: ${orderDate}`, 350, 74, { width: 200, align: "right" });
    ...
    doc
      ...
      .font("Helvetica-Bold")
      .fillColor("#0a1628")
      .text("Billing / Shipping Address:", 320, 118);
    doc
      .fontSize(9)
      .font("Helvetica")
      .fillColor("#333")
    ...
    doc
      ...
      .font("Helvetica-Bold")
      .fillColor("#0a1628")
      .text(`Order Number: #${orderNumber}`, 50, metaY);
    doc.text(`Payment Method: ${order.payment}`, 320, metaY);

    // Table header
    const tableTop = 270;
    doc
      .moveTo(50, tableTop - 8)
      .lineTo(550, tableTop - 8)
      .strokeColor("#e0e0e0")
      .lineWidth(1)
      .stroke();
...
router.post("/:id/ship-shiprocket", adminAuth, async (req, res) => {
  try {
    ...
    if (order.shiprocket && order.shiprocket.awbCode) {
      return res
        .status(400)
        .json({ error: "This order has already been shipped via Shiprocket" });
    }
    ...
    if (!shipmentData.shipment_id) {
      return res.status(500).json({
        error: shipmentData.message || "Failed to create Shiprocket shipment",
      });
    }
    ...
    try {
      const labelData = await shiprocket.generateLabel(
...
router.post("/courier-webhook", async (req, res) => {
  try {
    ...
    if (!order) {
      return res
        .status(200)
        .json({ message: "Order not found for this AWB, ignoring" });
    }
    ...
    if (mappedStatus && mappedStatus !== order.status) {
      ...
      await order.save();
      sendOrderStatusEmail(order, mappedStatus);
    }
  ...
  try {
    ...
    if (!["pending", "packed"].includes(order.status)) {
      return res.status(400).json({
        error:
          "This order can no longer be cancelled - it has already been shipped. Please contact us for help.",
      });
    }
