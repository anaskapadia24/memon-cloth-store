...
// Admin's size grid always sends all 8 size slots (S-5XL), even for
// unstitched/no-size items like dress material where every slot stays 0.
// If every size has 0 stock, treat it as "no sizes" so the storefront uses
// the flat stock field instead of getting stuck on "Select a Size" forever.
function normalizeSizes(parsedSizes) {
  if (!Array.isArray(parsedSizes)) return [];
  const totalStock = parsedSizes.reduce(
    (sum, s) => sum + (Number(s?.stock) || 0),
...
router.put(
  ...
  async (req, res) => {
    try {
      ...
      if (sizes !== undefined) {
        try {
          product.colors[colorIndex].sizes = normalizeSizes(
            typeof sizes === "string" ? JSON.parse(sizes) : sizes,
          );
        } catch (e) {
          // Keep existing sizes
